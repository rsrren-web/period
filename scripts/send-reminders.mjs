...
import {loadPeriods,loadUserData,model,addDays} from './period-core.mjs';
...
if(userData.settings?.ownerNotify!==false)await transporter.sendMail({from:`周期生活助手 <${process.env.GMAIL_USER}>`,to:process.env.OWNER_EMAIL,subject:`${prefix}明天接近预计经期`,text:`${testNote}明天接近本次经期的中心预测日期。建议今天确认用品、保证睡眠，并给明天的工作和运动留出调整空间。\n\n${common}`});
if(userData.settings?.partnerNotify!==false)await transporter.sendMail({from:`周期生活助手 <${process.env.GMAIL_USER}>`,to:process.env.PARTNER_EMAIL,subject:`${prefix}伴侣关怀提醒：明天可能接近经期`,text:`${testNote}明天可能接近预计经期。她在这个阶段可能更需要休息或安排缓冲，实际感受以她本人为准。建议先询问她今天希望获得怎样的支持。\n\n${common}`});
