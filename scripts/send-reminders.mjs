...
if(vancouverDate!==reminderDate){console.log(`今天 ${vancouverDate} 无需发送；下次检查目标 ${reminderDate}`);process.exit(0)}
...
await transporter.sendMail({from:`周期生活助手 <${process.env.GMAIL_USER}>`,to:process.env.OWNER_EMAIL,subject:'明天接近预计经期',text:`明天接近本次经期的中心预测日期。建议今天确认用品、保证睡眠，并给明天的工作和运动留出调整空间。\n\n${common}`});
await transporter.sendMail({from:`周期生活助手 <${process.env.GMAIL_USER}>`,to:process.env.PARTNER_EMAIL,subject:'伴侣关怀提醒：明天可能接近经期',text:`明天可能接近预计经期。她在这个阶段可能更需要休息或安排缓冲，实际感受以她本人为准。建议先询问她今天希望获得怎样的支持。\n\n${common}`});
console.log('本人和伴侣提醒已发送');
