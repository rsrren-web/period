...
const cacheVersion = sw.match(/period-helper-v(\d+)/)?.[1];
assert.ok(styleVersion, '样式必须带缓存版本');
assert.equal(styleVersion, appVersion, '样式与脚本缓存版本必须一致');
assert.equal(styleVersion, cacheVersion, '页面与 Service Worker 缓存版本必须一致');
