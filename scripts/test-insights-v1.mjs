...
assert.match(html, /insights-page\.js\?v=93/);
assert.match(serviceWorker, /period-helper-v93/);
assert.match(html, /Content-Security-Policy/);
assert.match(html, /connect-src 'self' https:\/\/period-sync\.rsr-ren\.workers\.dev/);
assert.match(app, /else if\(id==='calendar'\)\{renderCalendarEnhanced\(\);markStatusDates\(state\.logs\);renderWellnessEnhancements\(context,'calendar'\)/, '日历每次重绘后都必须直接恢复身体状态图标');
