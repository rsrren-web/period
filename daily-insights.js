...
function visibleSymptoms(log = {}) { return compatibilityTags(log).filter((item) => !item.startsWith('tcm:') && !item.startsWith('疼痛部位：') && !item.startsWith('入睡：') && !item.startsWith('排便：') && !['头痛', '腰腹不适'].includes(item)); }
