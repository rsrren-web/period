...
function statusCard(date, log) { if (!log) return '<section class="day-status-card empty"><strong>身体状态</strong><p>这一天还没有记录身体状态。</p></section>'; const ratings = [['情绪', tagged(log, '情绪：') || DAILY_LABELS[Number(log.mood) - 1] || '—'], ['精力', `${log.energy || '—'}/5`], ['睡眠', `${log.sleep || '—'}/5`], ['活动', `${log.activity || '—'}/5`], ['压力', `${log.stress || '—'}/5`], ['疼痛', `${painFive(log) ?? '—'}/5`]], locations = painParts(log), symptoms = visibleSymptoms(log).filter((item) => !item.startsWith('情绪：') && !item.startsWith('运动：') && !item.startsWith('社交：') && !item.startsWith('社交强度：') && !item.startsWith('社交影响：')), bedtime = tagged(log, '入睡：'), bowel = tagged(log, '排便：'), menstrual=menstrualRows(log),tcm=readTcmObservations(log.symptomTags),tcmLabels=[tcm.cold_sensation==='yes'?'明显怕冷':null,tcm.warmth_relief==='yes'?'热敷后缓解':null,tcm.nausea==='yes'?'恶心':null,tcm.diarrhea==='yes'?'腹泻':null,tcm.body_heaviness==='yes'?'身体沉重':null,Number.isFinite(tcm.bloating_level)?`腹胀 ${tcm.bloating_level}/5`:null,Number.isFinite(tcm.appetite_level)?`食欲 ${tcm.appetite_level}/5`:null].filter(Boolean); return `<section class="day-status-card"><div class="day-status-heading"><strong>身体状态记录</strong><span>${escapeDaily(date)}</span></div><div class="day-status-ratings">${ratings.map(([label, value]) => `<div><small>${label}</small><strong>${escapeDaily(value)}</strong></div>`).join('')}</div>${menstrual.map(([label,value])=>`<div class="day-status-row"><strong>${escapeDaily(label)}</strong><span>${escapeDaily(value)}</span></div>`).join('')}${bedtime ? `<div class="day-status-row"><strong>入睡时间</strong><span>${escapeDaily(bedtime)}入睡</span></div>` : ''}${bowel ? `<div class="day-status-row"><strong>排便</strong><span>${escapeDaily(bowel)}</span></div>` : ''}${locations.length ? `<div class="day-status-row"><strong>疼痛部位</strong><span>${locations.map(escapeDaily).join('、')}</span></div>` : ''}${tcmLabels.length?`<div class="day-status-row"><strong>体感观察</strong><span>${tcmLabels.map(escapeDaily).join('、')}</span></div>`:''}${symptoms.length ? `<div class="day-status-row"><strong>其他感受</strong><span>${symptoms.map(escapeDaily).join('、')}</span></div>` : ''}</section>`; }
...
function symptomLabels(log = {}) {
  ...
  if (Number.isFinite(bodySense.bloating_level) && bodySense.bloating_level >= 3) labels.push('腹胀明显');
...
function bodySenseAction(log = {}) {
  ...
  if (Number.isFinite(bodySense.bloating_level) && bodySense.bloating_level >= 4) return { id: 'bloating-care', severity: 6, reason: `腹胀 ${bodySense.bloating_level}/5，是今天较明显的体感`, text: '把一餐分小份慢慢吃，饭后舒缓走动10分钟' };
  return null;
}
