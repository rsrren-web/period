...
const MENSTRUAL_LABELS = {
  ...
  flow_level: { spotting: '点滴', light: '少', medium: '中', heavy: '多', very_heavy: '很多' },
  blood_color: { bright_red: '鲜红', dark_red: '暗红', brown: '棕色', pink: '淡', other: '其他' },
  clot_presence: { yes: '有', no: '没有', not_recorded: '未记录' },
  clot_level: { small: '少', medium: '中', large: '多' },
...
function menstrualSummary(log = {}) {
  ...
  if (log.menstrual_status === 'on_period' || log.menstrual_status === 'spotting_only') {
    ...
    const color = MENSTRUAL_LABELS.blood_color[log.blood_color];
    if (color) parts.push(color);
    if (log.clot_presence === 'no') parts.push('无血块');
    else if (log.clot_presence === 'yes') parts.push(`血块${log.clot_level === 'large' ? '多' : '少'}`);
  }
  return parts.join(' · ');
}
