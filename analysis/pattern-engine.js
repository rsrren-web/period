...
function insufficient(patternType, metric, qualities = [], cycles = 0) {
  return Object.freeze({ pattern_type: patternType, metric, sample_size: 0, cycles_covered: cycles, effect_size: null, confidence_level: 'insufficient', status: 'insufficient', data_quality: qualities });
}
...
export function analyzeCyclePattern({ logs = {}, periods = [], metric, as_of, target_window = { start_day: 1, end_day: 5 }, mode = 'mean', condition } = {}) {
  ...
  if (cycles.length < minimum) return insufficient('cycle_pattern', metric, [], cycles.length);
  ...
  if ([targetQuality, comparisonQuality].some(item => item.quality_level === 'insufficient')) return insufficient('cycle_pattern', metric, [targetQuality, comparisonQuality], cycles.length);
...
export function analyzeCoOccurrence({ logs = {}, periods = [], metric_a, metric_b, start, end, condition_a, condition_b } = {}) {
  ...
  if ([qualityA, qualityB].some(item => item.quality_level === 'insufficient')) return insufficient('co_occurrence', `${metric_a}:${metric_b}`, [qualityA, qualityB], cyclesCovered(dates, periods, end));
  const pairs = dates.map(date => [metricValue(logs[date], metric_a), metricValue(logs[date], metric_b), date]).filter(([a, b]) => a !== null && b !== null);
  if (pairs.length < ANALYSIS_CONFIG.patterns.association_min_pairs) return insufficient('co_occurrence', `${metric_a}:${metric_b}`, [qualityA, qualityB], cyclesCovered(pairs.map(pair => pair[2]), periods, end));
  const withA = pairs.filter(([a]) => matches(a, condition_a)), withoutA = pairs.filter(([a]) => !matches(a, condition_a));
  if (!withA.length || !withoutA.length) return insufficient('co_occurrence', `${metric_a}:${metric_b}`, [qualityA, qualityB], cyclesCovered(pairs.map(pair => pair[2]), periods, end));
...
export function analyzeTemporalAssociation({ logs = {}, periods = [], metric_a, metric_b, start, end, relation = 'same_day', condition_a, condition_b } = {}) {
  ...
  if ([qualityA, qualityB].some(item => item.quality_level === 'insufficient')) return insufficient('temporal_association', `${metric_a}:${relation}:${metric_b}`, [qualityA, qualityB], cyclesCovered(anchorDates, periods, end));
  ...
  if (pairs.length < ANALYSIS_CONFIG.patterns.association_min_pairs) return insufficient('temporal_association', `${metric_a}:${relation}:${metric_b}`, [qualityA, qualityB], cyclesCovered(pairs.map(pair => pair[2]), periods, end));
  const withA = pairs.filter(([a]) => matches(a, condition_a)), withoutA = pairs.filter(([a]) => !matches(a, condition_a));
  if (!withA.length || !withoutA.length) return insufficient('temporal_association', `${metric_a}:${relation}:${metric_b}`, [qualityA, qualityB], cyclesCovered(pairs.map(pair => pair[2]), periods, end));
  const pGivenA = probability(withA.map(([, b]) => matches(b, condition_b))), pGivenNotA = probability(withoutA.map(([, b]) => matches(b, condition_b))), effectSize = pGivenA - pGivenNotA, cycles = cyclesCovered(pairs.map(pair => pair[2]), periods, end);
  return Object.freeze({ pattern_type: 'temporal_association', metric: `${metric_a}:${relation}:${metric_b}`, metric_a, metric_b, relation, p_b_given_a: round(pGivenA), p_b_given_not_a: round(pGivenNotA), involved_days: { with_a: withA.length, without_a: withoutA.length }, sample_size: pairs.length, cycles_covered: cycles, effect_size: round(effectSize), confidence_level: confidence([qualityA.quality_level, qualityB.quality_level], pairs.length, cycles), status: detected(effectSize), data_quality: { metric_a: qualityA, metric_b: qualityB } });
}
