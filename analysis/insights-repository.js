...
function signature(input, configVersion) {
  return analysisFingerprint({ configVersion, logs: input.logs || {}, periods: input.periods || [], asOf: input.as_of, nextStart: input.next_start || null, predictionConfidence: input.prediction_confidence || null, interventionUsage: input.intervention_usage || [], constitutionProfile: input.constitution_profile || null, lifeStage: input.life_stage || 'regular', safetyProfile: input.safety_profile || {} });
}
